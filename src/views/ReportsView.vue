...
const reports = computed(() => {
  ...
  let items = resources.value.filter(r => r.type === 'report');
...
const categories = ['ทั้งหมด', 'ความปลอดภัย', 'การเงิน', 'การใช้ยา'];
...
<template>
  <div class="carta-container">
    <section class="cart-title-nav">
      <div class="cart-title cart-title--blue hover-shrink">
        <h2>รายงานและสถิติ</h2>
      </div>

      <div class="cart-nav cart-nav--blue rounded" aria-label="เมนูหมวดหมู่">
        <ul>
...
.cart-title {
  background: linear-gradient(135deg, var(--color-teal) 0%, var(--color-teal-claro) 100%);
  padding: 20px 6%;
  border-radius: 20px;
  display: flex;
  align-items: center;
